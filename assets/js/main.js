...
function cacheDOMElements() {
    ...
    DOM.ratingInputs = document.querySelectorAll('input[name="rating"]');
...
function initializeEventListeners() {
    ...
    DOM.ratingInputs.forEach(input => {
        input.addEventListener('change', (e) => {
...
async function loadTools() {
    ...
    try {
        ...
        allTools = [...tools1, ...tools2, ...tools3, ...tools4];
...
function updateActiveFiltersDisplay() {
    ...
    const ratingCount = filters.minRating > 0 ? 1 : 0;
    document.getElementById('rating-count').textContent = ratingCount > 0 ? ratingCount : '';
}
...
function loadFiltersFromURL() {
    ...
    if (params.has('rating')) {
        filters.minRating = parseFloat(params.get('rating'));
        const ratingInput = document.querySelector(`input[name="rating"][value="${filters.minRating}"]`);
...
function loadFiltersFromStorage() {
    try {
        ...
        filters.minRating = savedFilters.minRating || 0;
